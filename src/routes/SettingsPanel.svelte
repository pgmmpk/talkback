<script>
    ...
    let { visible = $bindable(), active = false } = $props();
</script>
{#if visible}
<div class="h-screen absolute top-0 left-0 w-3/4 border bg-white border-gray-700 z-2 flex flex-col shadow-xl shadow-gray-700 p-2" transition:fly={{ duration: 400, opacity: 1.0, x: '-100%' }}>
    ...
    <div class="flex flex-row items-center justify-center my-4">
            ...
            <input type="range" class="accent-gray-700 grow shrink" id="sensitivity" name="sensitivity" aria-label="Sensitivity" min="0.0" max="0.1" step="0.01" bind:value={settings.sensitivity} disabled={!active} />
    ...
    <div class="flex flex-row items-center justify-center my-4">
            ...
            <input type="range" class="accent-gray-700 grow" id="silence-threshold-millis" name="silence-threshold-millis" aria-label="Silence Threhold" min="100" max="1500" step="10" bind:value={settings.threshold} disabled={!active}/>
            <div class="w-12 ml-2 inline-block">{settings.threshold}</div>
    </div>
</div>
{/if}